...
conn = init_db()
def open_counsellor_signup(main_window):
    counsellor_signup_window = tb.Toplevel()
    counsellor_signup_window.title("Counsellor Sign Up")
    counsellor_signup_window.geometry("600x800")
    counsellor_signup_window.resizable(False,False)
    ...
    # -------UI Elements--------------
    register_frame = tb.Frame(counsellor_signup_window, padding=20)
    register_frame.pack(fill="both", expand=True)

    # --- Header ---
    header = tb.Label(register_frame, text="Counsellor Registration", font=("Helvetica", 20, "bold"), bootstyle="info")
    header.pack(pady=20)
    ...
    # Capture errors. eg database server is down
        try:
            ...
            cursor.execute("""
                    INSERT INTO counsellors_table (firstname, lastname, email, password)
                    VALUES (%s, %s, %s, %s)
                """, (firstname, lastname, email, hash_pw))

            conn.commit()
            # Selects user information from database
            cursor.execute("SELECT counsellor_id, firstname, lastname, email FROM counsellors_table WHERE email = %s AND password = %s", (email, hash_pw))
            result = cursor.fetchone()

            conn.close() # close DB connection

            counsellor_signup_window.destroy()  # Close the signup window
            open_counsellor_control_panel(result)  # Open the control panel after successful signup
    ...
    def cancel_signup():
        counsellor_signup_window.destroy()
        main_window.deiconify()  # Show the main window again

    # --------Button--------
    # signup button
    tb.Button(register_frame, text="Sign Up",
    command=counsellor_register, bootstyle="primary").pack(fill="x",
    pady=40, padx=100)
    # cancel button
    ...
    # create footer
    create_footer(counsellor_signup_window)

...
conn = init_db()
def open_senior_admin_signup(main_window):
    senior_admin_signup_window = tb.Toplevel()
    senior_admin_signup_window.title("Senior Signup Sign Up")
    senior_admin_signup_window.geometry("600x800")
    senior_admin_signup_window.resizable(False,False)
    ...
    # -------UI Elements--------------
    register_frame = tb.Frame(senior_admin_signup_window, padding=20)
    register_frame.pack(fill="both", expand=True)

    # --- Header ---
    header = tb.Label(register_frame, text="senior_admin Registration", font=("Helvetica", 20, "bold"), bootstyle="info")
    header.pack(pady=20)


    # function for customer registration
    def senior_admin_register():
    ...
    # Capture errors. eg database server is down
        try:
            ...
            cursor.execute("""
                    INSERT INTO senior_admin_table (firstname, lastname, email, password)
                    VALUES (%s, %s, %s, %s)
                """, (firstname, lastname, email, hash_pw))

            conn.commit()
            # Selects user information from database
            cursor.execute("SELECT admin_id, firstname, lastname, email FROM senior_admin_table WHERE email = %s AND password = %s", (email, hash_pw))
            result = cursor.fetchone()

            conn.close() # close DB connection

            senior_admin_signup_window.destroy()  # Close the signup window
            open_admin_control_panel(result)  # Open the control panel after successful signup
    ...
    def cancel_signup():
        senior_admin_signup_window.destroy()
        main_window.deiconify()  # Show the main window again

    # --------Button--------
    # signup button
    tb.Button(register_frame, text="Sign Up",
    command=senior_admin_register, bootstyle="primary").pack(fill="x",
    pady=40, padx=100)
    # cancel button
    ...
    # create footer
    create_footer(senior_admin_signup_window)
